...
public class DeviceDao implements Dao<Device>{
	...
	public List<Device> find() throws SQLException {
		return (List<Device>) qr.query("select * from client", new BeanListHandler(Device.class));
	}
	...
	public List<Device> findByDeviceToken(String device_token) throws SQLException{
		return (List<Device>) qr.query("select * from client where ap_id =(select ap_id from ap where dev_md5=?)", 
	...
	public Device findByMacAndDevId(Device device,String devId) throws SQLException {
		List<Device> query = (List<Device>) qr.query("select * from client where ap_id =(select ap_id from ap where dev_id=?) and mac=?", 
