...
public class RouterDao implements Dao<Router>{
	...
	public List<Router> find() throws SQLException {
		List<Router> query =  (List<Router>) qr.query("select * from ap_status",new BeanListHandler(Router.class));
		return query;
	}
	...
	public Router findByApid(Router router) throws SQLException{
		List<Router> query = (List<Router>) qr.query("select * from ap_status where ap_id = ?",
	...
	public List<Router> findByDeviceToken(String device_token) throws SQLException{
		List<Router> query = (List<Router>) qr.query("select * from ap_status where ap_id =(select ap_id from ap where dev_md5=?)", 
	...
	public Router findByDevId(String devId) throws SQLException {
		List<Router> query = (List<Router>) qr.query("select * from ap_status where ap_id =(select ap_id from ap where dev_id=?)", 
