...
public class HostDao implements Dao<Host>{
	...
	public List<Host> find() throws SQLException {
		return (List<Host>) qr.query("select * from rule_host", new BeanListHandler(Host.class));
	}
	...
	public List<Host> findByDeviceToken(String device_token) throws SQLException{
		return (List<Host>) qr.query("select * from rule_host where ap_id =(select ap_id from ap where dev_md5=?)", 
	...
	public List<Host> findByDevId(String devId) throws SQLException {
		return (List<Host>) qr.query("select * from rule_host where ap_id =(select ap_id from ap where dev_id=?)", 
