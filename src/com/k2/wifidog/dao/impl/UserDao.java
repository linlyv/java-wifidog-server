...
public class UserDao implements Dao<User>{
	...
	*/
	public boolean findbyUserName(String username) throws SQLException {
		List<User> users=(List<User>) qr.query("select * from ap_user where username=?",
				new String[]{username}, new BeanListHandler(User.class));
	...
	public List<User> find() throws SQLException {
		return (List<User>) qr.query("select * from ap_user", new BeanListHandler(User.class));
	}
	/**
	 * 修改用户
	 */
	...
	public User findUserByUsernamePasswordAndDevId(User u,String dev_id) throws SQLException{
		 List<User> users=(List<User>) qr.query("select * from ap_user where username=? and password=? and enable=? and ap_id=(select ap_id from ap where dev_id=?)",
				new String[]{u.getUsername(),u.getPassword(),"1",dev_id}, new BeanListHandler(User.class));
	...
	public List<User> findByDeviceToken(String device_token) throws SQLException{
		return (List<User>) qr.query("select * from ap_user where ap_id =(select ap_id from ap where dev_md5=?)", 
