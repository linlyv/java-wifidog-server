...
public class IPWhiteDao implements Dao<IPWhite>{
	...
	public List<IPWhite> find() throws SQLException {
		return (List<IPWhite>) qr.query("select * from rule_ipwhite", new BeanListHandler(IPWhite.class));
	}
	...
	public List<IPWhite> findByDeviceToken(String device_token) throws SQLException{
		return (List<IPWhite>) qr.query("select * from rule_ipwhite where ap_id =(select ap_id from ap where dev_md5=?)", 
	...
	public List<IPWhite> findByDevId(String devId) throws SQLException {
		return (List<IPWhite>) qr.query("select * from rule_ipwhite where ap_id =(select ap_id from ap where dev_id=?)", 
	...
	public IPWhite findByDeviceTokenAndIp(String deviceToken, String ip) throws SQLException {
		List<IPWhite> query = (List<IPWhite>) qr.query("select * from rule_ipwhite where ap_id =(select ap_id from ap where dev_md5=?) and ip=?", 
