...
public class MacBlackDao implements Dao<MacBlack>{
	...
	public List<MacBlack> find() throws SQLException {
		return (List<MacBlack>) qr.query("select * from rule_macblack", new BeanListHandler(MacBlack.class));
	}
	...
	public List<MacBlack> findByDeviceToken(String device_token) throws SQLException{
		return (List<MacBlack>) qr.query("select * from rule_macblack where ap_id =(select ap_id from ap where dev_md5=?)", 
	...
	public List<MacBlack> findByDevId(String devId) throws SQLException {
		return (List<MacBlack>) qr.query("select * from rule_macblack where ap_id =(select ap_id from ap where dev_id=?)", 
